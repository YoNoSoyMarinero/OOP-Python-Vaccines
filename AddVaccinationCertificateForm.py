...
class AddVaccinationCertificateForm(AddEditDataForm):
    ...
    def command_add(self):
        ...
        cert_date = self.VarD.get().split("/")
        cert_date = date(int(cert_date[0]), int(
            cert_date[1]), int(cert_date[2]))
        id = self.VarE.get()
        try:
            vaccination_certificate = VaccinationCertificate(
                id, cert_date, dose, citizen, medical_worker)
            self.data.add_vaccination_certificate(vaccination_certificate)
            self.data.save_data()
            self.cancle = False
            self.destroy()
